...
// 		StringCbCopy( HsExtError.szGameVersion, sizeof( HsExtError.szGameVersion ), STRFILEVER ); //Game 버전
		...
		AHNHS_EXT_ERRORINFO HsExtError = {0,};
		StringCbCopy( HsExtError.szServer, sizeof( HsExtError.szServer ), "203.89.147.220" ); //모니터링주소 
		StringCbCopy( HsExtError.szUserId , sizeof( HsExtError.szUserId ), "jal" ); //모니터링주소 
		StringCbCopy( HsExtError.szGameVersion, sizeof( HsExtError.szGameVersion ), STRFILEVER ); //Game 버전


#if INDONESIA

		int iRet = _AhnHS_StartMonitor(HsExtError, szFullFilePath);

		if( iRet != ERROR_SUCCESS )
		{
			if( pFile )
			{
				fprintf( pFile, "Monitor error:%d:%x\n", timeGetTime(), iRet );
				fflush( pFile );
				fclose( pFile );
			}
		}
...
bool ioHackShield::Start()
{
	...
	DWORD dwOption = AHNHS_CHKOPT_ALL | AHNHS_CHKOPT_LOCAL_MEMORY_PROTECTION | AHNHS_CHKOPT_DETECT_VIRTUAL_MACHINE | 
					AHNHS_CHKOPT_SEND_MONITOR_DELAY | AHNHS_CHKOPT_STANDALONE | AHNHS_CHKOPT_PROTECT_D3DX | 
	...
	__try
	{
		...
		int iRet = _AhnHS_Initialize ( szFullFilePath, 
			                           HS_CallbackProc, 
									   6054,                       // 게임 코드
									   "38AFFFEA877BEBFDA2549BF0", // 라이선스 키 
			                           dwOption,                   
			                           AHNHS_SPEEDHACK_SENSING_RATIO_NORMAL | AHNHS_SPEEDHACK_EXOPT_ALLOW_SLOW ); 
...
bool ioHackShield::OnRecieveCheck( IN HackShieldPacket &rRecvBuf, OUT HackShieldPacket &rSendBuf )
{
	__try
	{
		int ulRet = _AhnHS_MakeResponse ( rRecvBuf.byBuffer, rRecvBuf.nLength, (_AHNHS_TRANS_BUFFER*)&rSendBuf ); 
...
void ioHackShield::SetUserID(const ioHashString &rsPrivateID)
{
	_AhnHS_SetUserId(rsPrivateID.c_str());
}
