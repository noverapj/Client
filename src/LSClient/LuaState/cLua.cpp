...
cLua::cLua()
{
	...
	if( m_pScriptContext == NULL )
	{
		LOG.PrintTimeAndLog( 0, "LuaContext NULL" );
		return;
	}
...
bool cLua::RunScript(const char *pFname)
{
	...
	if (0 != luaL_loadfile(m_pScriptContext, pFilename))
	{
		...
		char buf[256];
		sprintf(buf, "Lua Error - Script Load\nScript Name:%s\nError Message:%s\n", pFilename, luaL_checkstring(m_pScriptContext, -1));
		LOG.PrintTimeAndLog( 0, "cLua::LoadFile Error - %s", buf );
	...
	if (0 != lua_pcall(m_pScriptContext, 0, LUA_MULTRET, 0))
	{
		...
		char buf[256];
		sprintf(buf, "Lua Error - Script Run\nScript Name:%s\nError Message:%s\n", pFilename, luaL_checkstring(m_pScriptContext, -1));
		LOG.PrintTimeAndLog( 0, "cLua::Call Error - %s", buf );
...
bool cLua::RunMemory( const char *pBuffer, const int iBufferSize )
{
	if (0 != luaL_loadbuffer(m_pScriptContext, pBuffer, iBufferSize, NULL))
	{
		if(m_pErrorHandler)
		{
			char buf[256];
...
int cLua::LuaExceptionProcess( lua_State * pLua )
{
	const char * pStr = luaL_checkstring(pLua, -1);
	LOG.PrintTimeAndLog(0,"[LUA_EXCEPTION] script error message - %s", pStr);
	throw;
}
